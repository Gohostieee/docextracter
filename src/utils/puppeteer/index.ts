...
export async function initPuppeteer(): Promise<Browser> {
  try {
    ...
    console.log('Browserbase browser connected successfully');

    return browser;
  } catch (error) {
    console.error('Failed to initialize Browserbase browser:', error);
...
/**
 * Auto-scroll page to trigger lazy-loaded content (Playwright)
 */
...
/**
 * Force viewport rendering with extreme dimensions (Playwright)
 */
...
/**
 * Fetch a page and extract all links using Browserbase (Playwright)
 */
export async function extractLinksWithPuppeteer(
  ...
  try {
    ...
    // Try to wait for network idle (Playwright API)
    try {
...
/**
 * Fetch HTML content using Browserbase (Playwright) with optional aggressive rendering
 *
 * Non-aggressive mode (fast): Force viewport + networkidle + single network idle check (~5-10s)
...
export async function fetchHTMLWithPuppeteer(
  ...
  try {
    ...
    // Navigate to the page with extended timeout and wait for networkidle (Playwright API)
    await page.goto(url, {
